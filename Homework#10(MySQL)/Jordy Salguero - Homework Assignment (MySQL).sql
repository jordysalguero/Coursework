...
SELECT (CONCAT(first_name, " ", last_name)) AS Staff, SUM(amount) AS "Total Amount ($)" FROM staff 
...
SELECT COUNT(inventory_id) AS "# of Copies" FROM inventory 
	WHERE film_id IN 
...
SELECT title AS "Film", count(rental_date) AS "Rental Frequency" FROM film 
...
SELECT CONCAT(address, "," , " " , city, ",", " ", country) AS "Store Address", SUM(amount) AS "Store Amount" FROM payment 
...
SELECT store_id AS "ID", city AS "City", country AS "Country" FROM store 
	JOIN address ON store.address_id = address.address_id 
		JOIN city ON city.city_id = address.city_id 
			JOIN country ON country.country_id = city.country_id;
